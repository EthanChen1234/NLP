...
vali_epoch_steps = int(len(data_vali) / BATCH_SIZE) - 1


def add_layer(inputs, in_size, out_size, activation_function=None):
    w_limit = np.sqrt(6.0 / (in_size + out_size))
    weights = tf.Variable(tf.random_uniform([in_size, out_size], -w_limit, w_limit))
    biases = tf.Variable(tf.random_uniform([out_size], -w_limit, w_limit))
...
def get_cosine_score(query_arr, doc_arr):
    ...
    cos_scores = tf.math.divide(pooled_mul_12, pooled_len_1 * pooled_len_2 + 1e-8, name="cos_scores")
    return cos_scores
...
with tf.name_scope('Drop_out'):
    query_l1 = tf.nn.dropout(query_l1, rate=1-keep_prob)
    doc_l1 = tf.nn.dropout(doc_l1, rate=1-keep_prob)
...
def pull_batch(data_set, batch_id):
    """data_set, [[prefix, title, label], ...]"""
    data_batch = data_set[batch_id*BATCH_SIZE: (batch_id+1)*BATCH_SIZE]
    query_in = [data[0] for data in data_batch]
    doc_in = [data[1] for data in data_batch]
